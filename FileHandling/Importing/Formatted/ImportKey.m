classdef ImportKey
    ...
    %% External Methods
    methods (Static)
        function map = alias_lookup(fileKey, partialName)
            map = ImportKey.dirMap_make(fileKey);
            return
            ...
            alias = string(fileKey.alias{irow}{1});
            subName = string(fileKey.subName{irow});
        end
    end
    
    %% Lookup Methods
    methods (Static, Access = private)
        function map = dirMap_make(fileKey)
            map = table();
